...
sed -i '/HIP_DISABLE_WARP_SYNC_BUILTINS/c\#if HIP_VERSION >= 70000000\n#define HIP_ENABLE_WARP_SYNC_BUILTINS 1\n#else\n#define HIP_DISABLE_WARP_SYNC_BUILTINS 1\n#endif' "$VENDOR_HIP_FILE"
...
EOF
    ...
    # Skip the original __<xxxx>_sync lines and print the rest
    tail -n +$((SHFL_LINE + 5)) "$VENDOR_HIP_FILE"
    
} > "$VENDOR_HIP_FILE.tmp"

mv "$VENDOR_HIP_FILE.tmp" "$VENDOR_HIP_FILE"
#grep -v _sync "$VENDOR_HIP_FILE.tmp" > "$VENDOR_HIP_FILE"
#sed -i 's/HIP_DISABLE_WARP_SYNC_BUILTINS/HIP_ENABLE_WARP_SYNC_BUILTINS/g' "$VENDOR_HIP_FILE"
